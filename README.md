...
Maintains a window (subarray/substring) that expands or shrinks as you iterate. Avoids recomputing from scratch on every step.
...
**When to use:** Contiguous subarray/substring problems — max sum, min length, longest without repeating, etc.
...
**When to use:** Calendar/scheduling problems, overlapping ranges, meeting rooms.
